...
{
	CODBCmd cmd;
	if (cmd.Init(gserver.m_dbconn))
	{
#ifdef LC_TLD
		cmd.Open("SELECT TOP 1 * FROM [bill_user]");
...
void ReadReserveInfo(int userindex, CUser& user)
{
	...
	CODBCmd selcmd;
	if (!selcmd.Init(gserver.m_dbconn))
	{
#ifdef BSTEST
	GAMELOG << init("DO", strrchr(__FILE__, '\\') + 1) << __LINE__ << end;
#endif
	...
	GAMELOG << init("DO", strrchr(__FILE__, '\\') + 1) << __LINE__ << end;
#endif
			if (selcmd.Get(1, rbguid) && selcmd.Get(2, rbvalue) && selcmd.Get(3, rbname))
			{
...
bool ReadCurrentInfo(int userindex, CUser& user, bool bForce)
{
	...
	try
	{
		DBCHECK(selcmd.Get(1, billguid));
		DBCHECK(selcmd.Get(2, surpluspoint));
		DBCHECK(selcmd.Get(3, billsection));
		DBCHECK(selcmd.Get(4, billviewname));

		if (!selcmd.IsNULL(5))
		{
			DBCHECK(selcmd.Get(5, increasedays));
		}
		else
		{
			SYSTEMTIME st;
...
bool ReadSectionInfo(const char* bguid, CSectionInfo& section)
{
	...
	{
		DBCHECK(selcmd.Get(1, billguid));
		DBCHECK(selcmd.Get(2, billsection));
		DBCHECK(selcmd.Get(3, billhour));
		DBCHECK(selcmd.Get(4, billvalue));
		DBCHECK(selcmd.Get(5, billviewname));
		DBCHECK(selcmd.Get(6, billevent));
...
bool ReadGamePoint(int userindex, CGamePointList& list)
{
	...
	while (selcmd.MoveNext() == S_OK)
	{
		if (selcmd.Get(1, itemguid) && selcmd.Get(2, surplus) && selcmd.Get(3, sessionguid))
		{
...
bool UpdateSupItem(CSupUpdateItemList &supUpdateItemList)
{
	...
	CODBCmd updatecmd;
	if (!updatecmd.Init(gserver.m_dbconn) )
	{
		printf("디비 커맨더 생성 실패");
		return false;
	}
...
bool ReadSupItem(int userindex, CSupItemList &list, int serverGroup)
{
	...
	while (selcmd.MoveNext() == S_OK)
	{
		if (selcmd.Get(1, index) 
			&& selcmd.Get(2, catalogid) 
			&& selcmd.Get(3, server) 
			&& selcmd.Get(4, itemindex)
			&& selcmd.Get(5, itemplus)
			&& selcmd.Get(6, itemflag)
			&& selcmd.Get(7, itemcount)
			)
		{
			list.Add( index, 
					catalogid,

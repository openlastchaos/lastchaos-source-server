...
int GetBalance(const char* idname)
{
	...
	if (!selcmd.Open(g_buf) || selcmd.MoveNext() != S_OK)
	{
		GAMELOG << init("BALANCE_REQUEST_ERROR")
				<< "DATABASE_OPEN"  << delim
				<< idname  << delim
...
int GetBalance(int userindex, const char *idname)
{
	...
	selcmd.Get("user_id", name);
	TrimString(name);
	selcmd.Get("user_point", balance);

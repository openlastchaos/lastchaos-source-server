...
public:
	CPC*				m_ch;
	int					m_tab;
	CInventoryRow*		m_rows[MAX_INVENTORY_ROWS];
	...
	{
		int r, c;
		if (FindItem(&r, &c, nItemIndex))
		{
			const CItem* pItem = m_rows[r]->m_items[c];
			return pItem->GetItemLevel(bOriginalLevel);
		}
	...
	// 아이템 찾기
	bool FindItem(int* row, int* col, int itemdbindex, int plus, int flag) const;	// itemproto index 
	bool FindItem(int* row, int* col, int itemidx) const;							// item index

...
#define MST_ASSIST_EXPLOSION		39			// 자폭(npc)
...
#endif // NIGHTSHADOW_SKILL
#ifdef RAID_MONSTER_SKILL
#define MST_OTHER_CALL			18			// 호출(npc가 주위 동족에게)
#endif // RAID_MONSTER_SKILL
...
#ifdef NIGHTSHADOW_SKILL
#define MST_AURA_DARKNESS		1			// 공격력 감소 
#define MST_AURA_WEAKNESS		2			// 방어력 감소 
#define MST_AURA_ILLUSION		3			// 크리티컬 상승
#endif // NIGHTSHADOW_SKILL
...
#ifdef NIGHTSHADOW_SKILL
#define AST_CONFUSED			(1 << MST_ASSIST_CONFUSED)			// 몬스터만 혼란에 걸리게 함
#define AST_TAMING				(1 << MST_ASSIST_TAMING)			// 몬스터를 일정시간 아군으로 만듬
#define AST_FREEZE				(1 << MST_ASSIST_FREEZE)			// 결계 (서로 공격 못함, 걸린 것은 이동 못함)   
#define AST_INVERSE_DAMAGE		(1 << MST_ASSIST_INVERSE_DAMAGE)			// 피격당하면 데미지 만큼 HP 회복   
#define AST_HP_DOT				(1 << MST_ASSIST_HP_DOT)			// 지속데미지 입음
#endif // NIGHTSHADOW_SKILL
#ifdef RAID_MONSTER_SKILL
#define AST_REBIRTH				(1 << MST_ASSIST_REBIRTH)			// 죽을때 이 버프가 있으면 바로 부활
#define AST_EXPLOSION			(1 << MST_ASSIST_EXPLOSION)			// 이 버프가 사라지면 자폭
#endif // RAID_MONSTER_SKILL
...
#else
	CMagicProto(int index, char maxlevel, char type, char subtype, char damagetype, char hittype, char attribute, int psp, int ptp, int hsp, int htp, CMagicLevelProto** levelproto);
#endif // NIGHTSHADOW_SKILL
...
public:
	...
	~CSkillListNode() { if (m_skill) delete m_skill; m_skill = NULL; }

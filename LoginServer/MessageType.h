...
typedef enum _tagMsgSysType
{
	...
	MSG_SYS_OVER_WEIGHT,			// 10 무게가 오바 瑛?	MSG_SYS_UPGRADE_NOCONDITION,	// 조건이 안맞아 업그레이드 못함 (일반제련석 레벨이 맞지 않음)
...
typedef enum _tagEventLattoType
{
	...
	MSG_EVENT_LATTO_BLOOD_SUCCESS,				// "블러드 역전 라??아이템 옵션에 당첨 되었습니다"
...
typedef enum _tagGuildErrorType
{
	...
	MSG_GUILD_ERROR_OUTBOSS,					// 길드장이 탈퇴하려면 紵?	MSG_GUILD_ERROR_OUT_OK,						// 탈퇴 성공
...
typedef enum __tagMsgRaidErrorType
{
	...
	MSG_RAID_ERROR_INZONE_JOIN_FULL,		// 인존 인원 풀
}MSG_RAID_ERROR_TYPE;
